...
class TestAddPhotoLoRA:
    def test_adds_adapters(self):
        ...
        photo = pkl.add_photo_lora(
            model, rank=2, n_basis=4, target_modules=["q_proj", "v_proj"], noise_sim=False
        )
    ...
    def test_output_shape_preserved(self):
        model = _TinyTransformer()
        photo = pkl.add_photo_lora(
            model, rank=2, n_basis=4, target_modules=["q_proj"], noise_sim=False
        )
        x = torch.randn(3, 16)
