...
class TestGradcheckActivation:
    ...
    def test_gradcheck_passes(self, ActClass, n_basis):
        ...
        assert result["passed"], f"{ActClass.__name__} gradcheck failed: {result.get('error', '')}"
...
class TestGradcheckLayer:
    def test_sine_layer_gradcheck(self):
        layer = PhotoKANLayer(2, 2, activation="sine", backend="cpu", noise_sim=False, n_basis=4)
    ...
    def test_relu_layer_gradcheck(self):
        layer = PhotoKANLayer(2, 2, activation="relu", backend="cpu", noise_sim=False, n_basis=4)
        result = gradcheck_layer(layer, atol=1e-2)
