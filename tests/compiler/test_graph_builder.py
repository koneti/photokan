...
from photokan.compiler import ExecGraph, LUTCompiler, QPALGraphBuilder
from photokan.layers import PhotoKAN


def _make_model():
    return PhotoKAN([2, 4, 1], activation="sine", backend="cpu", noise_sim=False, n_basis=4)


def _compile_luts(model):
    return LUTCompiler(n_points=64, max_mse=1e-2).compile_model(model, validate=False)
