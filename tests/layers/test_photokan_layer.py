...
class TestPhotoKANLayer:
    ...
    def test_output_shape(self, activation):
        layer = PhotoKANLayer(4, 3, activation=activation, backend="cpu",
    ...
    def test_parameter_count(self):
        layer = PhotoKANLayer(2, 3, activation="sine", n_basis=4,
                               backend="cpu", noise_sim=False)
    ...
    def test_wrong_backend_raises(self):
        ...
        from photokan.backend import PhotonicBackendError
        layer = PhotoKANLayer(2, 2, activation="sine", backend="qpal",
                               noise_sim=False, n_basis=4)
        x = torch.randn(4, 2)
    ...
    def test_noise_sim_enabled(self):
        layer_noisy = PhotoKANLayer(2, 2, activation="sine", backend="cpu",
                                    noise_sim=True, n_basis=4)
        layer_clean = PhotoKANLayer(2, 2, activation="sine", backend="cpu",
                                    noise_sim=False, n_basis=4)
